...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: 'Internal Server Error', message: 'Failed to fetch user preferences' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const validPreferences = ['apiUsageAlerts', 'securityAlerts', 'dataUpdateAlerts'];
    ...
    const updatedPreferences = await userPreferenceService.updatePreferences(user.id, preferences);
    ...
    return NextResponse.json(
      { error: 'Internal Server Error', message: 'Failed to update user preferences' },
      { status: 500 }
    );
  }
} 