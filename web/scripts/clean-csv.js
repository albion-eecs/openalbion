    ...
    .forEach(row => {
      ...
      for (let yearIdx = 0; yearIdx < years.length; yearIdx++) {
        ...
        if (
          ...
          (row[dataStartIdx] || row[dataStartIdx + 1])
        ) {
          cleanedData.push([
            years[yearIdx],
            currentCategory,
            ...
            row[dataStartIdx + 1]
              ? row[dataStartIdx + 1].toString().trim()
              : "0",
          ]);
        }
      }
...
function cleanGenericData(data) {
  ...
  if (
    ...
    nonEmptyRows[1].filter(Boolean).length >
      nonEmptyRows[0].filter(Boolean).length
  ) {
    headerRowIndex = 1;
  }
